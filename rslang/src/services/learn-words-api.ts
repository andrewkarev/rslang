...
class LearnWordsApi {
  ...
  constructor() {
    this.root = "https://rslang-project-2022.adaptable.app";
  ...
  private async createRequest<T>(
    url: string,
    options?: IRequestFunctionOptions
  ): Promise<T | void> {
  ...
  public async getUser(
    id: string
  ): Promise<Pick<IUser, "name" | "email" | "id"> | void> {
  ...
  private async getNewUserTokens(id: string): Promise<IUserToken | void> {
    try {
      ...
      if (response.status === 401)
        throw new AuthorisationError("Refresh token expired.");
